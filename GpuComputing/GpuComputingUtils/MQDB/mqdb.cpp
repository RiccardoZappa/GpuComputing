 ...
 */
int Matrix::genRandDims(mqdb* M, uint n, uint k, int seed) {
 ...
 */
void Matrix::fillBlocks(mqdb* M, uint n, uint k, char T, float c) {
	//mat size n*n
	...
	// set description
	sprintf_s(M->desc,sizeof(M->desc), "Random mqdb:  mat. size = %d, num. blocks = %d", n, k);
}
 ...
 */
int Matrix::genRandDimsUnified(mqdb* M, uint n, uint k, int seed) {
 ...
 */
void Matrix::fillBlocksUnified(mqdb* M, uint n, uint k, char T, float c) {
	//mat size n*n
	M->nElems = 0;
	int offset = 0;
	// loop on blocks
 ...
 */
MQDB::Matrix::mqdb genRandMat(unsigned n, unsigned k, unsigned seed) {
	MQDB::Matrix::mqdb M;
	Matrix m;
	m.genRandDims(&M, n, k, seed);
	// random fill mat entries
	m.fillBlocks(&M, n, k, 'R', 1.0);
 ...
 */
MQDB::Matrix::mqdb Matrix::mqdbConst(uint n, uint k, uint seed, float c) {
	mqdb M;
 ...
 */
void Matrix::mqdbProd(mqdb A, mqdb B, mqdb C) {
	uint n = 0;
 ...
 */
void Matrix::matProd(mqdb A, mqdb B, mqdb C) {
	int n = 0;
	for (uint i = 0; i < A.nBlocks; i++)
		n += A.blkSize[i];
 ...
 */
void Matrix::checkResult(mqdb A, mqdb B) {
 ...
 * print mqdb
 */
void Matrix::mqdbDisplay(mqdb* M) {
	int k = M->nBlocks;
	int n = 0;
	printf("%s\n", M->desc);
